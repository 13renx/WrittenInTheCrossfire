...
json Client::fetchResponse(const std::string& promptType) {
	cpr::Response res;

	if(promptType == "test") {
		res = cpr::Post(cpr::Url{ "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent" },
			cpr::Parameters{ { "key", this->apiKey } },
			cpr::Header{ { "Content-Type", "application/json" } },
			cpr::Body{ this->testPrompt.dump() });
	}
	else if(promptType == "game") {
		res = cpr::Post(cpr::Url{ "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent" },
			cpr::Parameters{ { "key", this->apiKey } },
			cpr::Header{ { "Content-Type", "application/json" } },
			cpr::Body{ this->gamePrompt.dump()});
	}
